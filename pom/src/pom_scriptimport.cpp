...
//        cerr << id << endl;
        ...
        PomTranslationSheetEntry str = scriptSheet.getStringEntryById(id);
...
int main(int argc, char* argv[]) {
  if (argc < 1) {
    cout << "Community Pom script importer" << endl;
    cout << "Usage: " << argv[0]
      << endl;
    return 0;
  }
  
  scriptSheet = PomTranslationSheet();
//  scriptSheet.importCsv("script/script_main.csv");
  scriptSheet.importCsv("script/pom_script.csv");
  importScript("spec_main.txt", true);
  
//  scriptSheet = PomTranslationSheet();
//  scriptSheet.importCsv("script/script_system.csv");
  importScript("spec_system.txt", true);
  
//  scriptSheet = PomTranslationSheet();
//  scriptSheet.importCsv("script/script_sjis.csv");
  importScript("spec_sjis.txt", true);
  
  importScript("spec_new.txt");
  
  importScript("spec_end.txt");
  
  importScript("spec_endnew.txt");
