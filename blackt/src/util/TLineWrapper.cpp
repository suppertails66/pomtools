...
//      && (ifs.peek() == '<')) {
  if (ifs.peek() == literalOpenSymbol()) {
    ...
    if (!ifs.eof() && ifs.peek() == '$') {
      ...
      while (!ifs.eof() && ifs.peek() != literalCloseSymbol())
        valuestr += ifs.get();
//      valuestr += ifs.get();
...
//    std::cerr << ifs.peek() << thingy.matchTableEntry(ifs).id << std::endl;
//    ifs.seek(pos);
  }
...
//      litstr += ifs.get();
      ...
      if (!ifs.eof() && ifs.peek() == literalCloseSymbol()) {
        litstr += ifs.get();
//        currentWordBuffer.write(litstr.c_str(), litstr.size());
...
char TLineWrapper::literalOpenSymbol() const {
  return defaultLiteralOpenSymbol;
}

char TLineWrapper::literalCloseSymbol() const {
  return defaultLiteralCloseSymbol;
}
